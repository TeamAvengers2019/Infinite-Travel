...
function initGame(){
    ...
    gameOverText = new transparentButton(new component(850, 200 , "rgba(0, 0, 0, 0.6)", 350, 175, "roundRect"), new component("20px", "'Press Start 2P'", "white", 400, 260, "text"));
    ...
    Poppicswitch = new transparentButton(new component(70, 30 , "rgba(0, 0, 0, 0.7)", 1500, 30, "roundRect" , 15), new component(26, 26 , "rgba(255, 255, 255, 0.7)", 1505, 33, "roundRect" , 13));
    PoppicswitchText = new component("15px", "'Press Start 2P'", "black", 1350, 90, "text");
...
function component(width, height, color, x, y, type, shape) {
    ...
    this.clicked = function() {
        ...
        if(myGameArea.clickGap>0){
            clicked = false;
        }
        if (clicked == true){
            console.log("An element is clicked")
            myGameArea.clickGap = 15;
        }
    ...
    this.moveTo =function(targetX, targetY){
        x = responsive_w(targetX);
        y = responsive_h(targetY);
        distanceX = Math.abs(x - this.x);
        distanceY = Math.abs(y-this.y);
        if(distanceX >= Math.abs(this.speedX)){
            this.speedX = (x-this.x)/5;
        } else {
            this.x = x;
            this.speedX = 0;
        }
...
function gameStart() {
    ...
    switch(mypoppic){
        case false:
            Poppicswitch.content.moveTo(1502,30);
            Poppicswitch.background.color = "rgba(0,0,0,0.7)";
            Poppicswitch.content.color = "rgba(255,255,255,0.7)";
            Poppicswitch.update();
            PoppicswitchText.text = "POP PICTURE OFF"
            PoppicswitchText.update();
            break;
        case true:
            Poppicswitch.content.moveTo(1542,33);
            Poppicswitch.background.color = "rgba(255,255,255,0.9)";
            Poppicswitch.content.color = "rgba(48,48,48,0.9)";
            Poppicswitch.update();
            PoppicswitchText.text = "POP PICTURE ON"
            PoppicswitchText.update();
            break;
    }
    ...
    if (myGameArea._x && myGameArea._y) {               //hovering effects
        ...
        if(difficultyEasy.background.hovered() || difficultyNormal.background.hovered() ||difficultyLunatic.background.hovered() || Poppicswitch.background.hovered()){
    ...
    if(myGameArea.x && myGameArea.y && Poppicswitch.background.clicked()){
        if(mypoppic){
            mypoppic = false;
        } else {
            mypoppic = true;
...
function gameOver(){
    ...
    gameOverText.content.text= "GAME OVER   " + "YOUR SCORE = " + finalScore ;
    gameOverText.update();
